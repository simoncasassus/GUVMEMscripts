...
load_path_4scripts = os.environ['HOME']+'/common/python/include/GUVMEMscripts/Lines/'


def store_outputcube(datacube,hdu_canvas,VectorBeam,beamdata,masterlabel,tagfileout='model_cube'):
...
def execute_1chan(atask):
    ...
    if DoGUVMEMRUN:
        command=path_to_guvmem+"  -X 16 -Y 16 -V 256 "+dogrid+" -i "+channelms+" -o "+workdir+"/out_res_ms --noise_cut "+str(noisecut)+" -m mod_in_0.fits -p "+workdir+"/ -O "+workdir+"/mod_out.fits  "+graphic_card+" -z "+str(MINPIX)+" -Z "+str(lbdaS)+","+str(lbdaL)+" -t "+str(MaxNiter)+" --verbose -e "+str(eta)+" "+positivflag+" "+printflag
        if DoMask:
            command+=' -U '+maskname
        if UVtaper:
...
def exec_cuberun(sourcems, cellsize, nxin, nyin, lbdaS=0.0, lbdaL=0.0, MakeCanvasCube=True, FileCanvasCube='cube_canvas.fits', MINPIX=0., DoL1=False, Grid=False, PrintImages=False,PositivDefinit=True,MaxNiter=80,XtraNameTag='',spws='0',fields='0',nchan1=0,nchan2=-1,Extract_Channel=True,Gen_ImageCanvas=True,CleanUpChannels=False,CleanUp=False,DoGUVMEMRUN=True,DoRestore=True,robustparam='1.0',cards=['0','1','2','3','4','5'],noisecut=2.,DoMask=False,UVtaper=False):
    ...
    if MakeCanvasCube:
        command='casa --log2term --nogui -c '+load_path_4scripts+'run_dirtymap.py'+' '+sourcems+' '+cellsize+' '+str(nxin)+' '+str(nyin)+' '+str(spws)+' '+robustparam+' '+FileCanvasCube
    ...
    if Extract_Channel:
        for ichan in range(nchan1,nchan2+1):
            ...
            genchannelms=1
            genclean=0
            os.system('casa --log2term --nogui -c '+load_path_4scripts+'xtract_1chan.py'+' '+sourcems+' '+fields+' '+str(spws)+' '+str(ichan)+' '+cellsize+' '+str(nxin)+' '+str(nyin)+' '+robustparam+' '+str(genclean)+' '+str(genchannelms))
    ...
    if Gen_ImageCanvas:
        for ichan in (nchan1,nchan2):
            ...
            data_canvas_1chan = fits.open('clean_channel_'+str(ichan)+'.fits')
    ...
    mastertasks={'sourcems':sourcems,
                 'masterlabel':masterlabel,
                 'options':{'Extract_Channel':Extract_Channel,
                            'fields':fields,
                            'spws':spws,
                            'cellsize':cellsize,
                            'nxin':nxin,
    ...
    for ibatch in list(range(nbatches)):
        batches.append({'ichan_start':ibatch*ncards+nchan1,'ncards':ncards})
        
    if (remainder > 0):
        print("THERE IS A REMAINDER:",remainder)
        batches.append({'ichan_start':nbatches*ncards+nchan1,'ncards':remainder})
    ...
    store_outputcube(cube_model,hdu_canvas,VectorBeam,beamdata,masterlabel,tagfileout='model_cube')

    masterlabel+='_robust'+str(robustparam)

    store_outputcube(cube_restored,hdu_canvas,VectorBeam,beamdata,masterlabel,tagfileout='restored_cube')
    store_outputcube(cube_residuals,hdu_canvas,VectorBeam,beamdata,masterlabel,tagfileout='residual_cube')
